...
class PlotPanel(wx.Panel):
    ...
    def populate_middle(self, sizer=None):
        ...
        self.lctrlLines.plotpanel = self
    ...
    def populate_bottom(self, sizer=None):
        ...
        # Bind X/Y scale radioboxes and threshhold labels
        self.Bind(wx.EVT_RADIOBOX, self.onXscale, self.rbXscale)
        self.Bind(wx.EVT_RADIOBOX, self.onYscale, self.rbYscale)
        self.Bind(wx.EVT_TEXT_ENTER, self.onXthresh,
                  self.txtXthresh)
        self.Bind(wx.EVT_TEXT_ENTER, self.onYthresh,
                  self.txtYthresh)
    ...
    def update_legend(self):
        """
        Updates the legend to reflect changes in the model.
        """
        checked = self.chkLegend.IsChecked()
        if checked:
    ...
    def update_figure(self):
        """
        Updates the figure in the graphframe.
        """
    ...
    def onXthresh(self, event):
        print "in onXthresh" #DELME
        xscale = self.rbXscale.GetSelection()
        if xscale == 2:
            try:
                linthreshx = float(self.txtXthresh.GetValue())
            except ValueError:
                ...
                self.txtXthresh.ChangeValue('1.e-4')
                self.txtXthresh.SetInsertionPoint(-1)
            self.plotter.xscale('symlog', linthreshx=linthreshx)
            self.plotter.autorange()
            self.update_figure()

    def onYthresh(self, event):
        print "in onYthresh" #DELME
        yscale = self.rbYscale.GetSelection()
        if yscale == 2:
            try:
                linthreshy = float(self.txtYthresh.GetValue())
            except ValueError:
                ...
                self.txtYthresh.ChangeValue('1.e-4')
                self.txtYthresh.SetInsertionPoint(-1)
            self.plotter.yscale('symlog', linthreshy=linthreshy)
            self.plotter.autorange()
            self.update_figure()
